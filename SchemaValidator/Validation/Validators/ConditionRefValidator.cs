...
public class ConditionRefValidator : Validator
{
	...
	public ConditionRefValidator(IGameFileAccess gameData) : base(gameData) { }

	public ConditionRefValidator(IGameFileAccess gameData, Dictionary<string, List<int>> ignoredValues) : base(gameData)
	{
	...
	public override ValidationResults Validate(ExcelHeaderFile exh, Sheet sheet)
	{
		...
		foreach (var linkData in linkVals.Distinct())
		{
			...
			{
				var tmpDataFile = GameData.GetRawExcelSheet(sheetName);
				if (tmpDataFile == null)
				{
