...
public class SingleLinkRefValidator : Validator
{
	...
	public SingleLinkRefValidator(IGameFileAccess gameData) : base(gameData) { }

	public SingleLinkRefValidator(IGameFileAccess gameData, Dictionary<string, List<int>> ignoredValues) : base(gameData)
	{
	...
	public override ValidationResults Validate(ExcelHeaderFile exh, Sheet sheet)
	{
		...
		var dataFile = GameData.GetRawExcelSheet($"{sheet.Name}");
		if (dataFile == null)
		{
		...
		foreach (var linkData in linkVals.Distinct())
		{
			...
			var linkedDataFile = GameData.GetRawExcelSheet(sheetName);
