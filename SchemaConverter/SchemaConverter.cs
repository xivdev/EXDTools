...
using Old = EXDCommon.SchemaModel.SaintCoinach;
using New = EXDCommon.SchemaModel.EXDSchema;
...
public class SchemaConverter
{
	...
	public static bool Convert(ExcelHeaderFile exh, string oldSchemaPath, string newSchemaPath)
	{
		...
		foreach (var col in columnInfos)
		{
			var field = new New.Field
			{
				...
				{
					"quad" => New.FieldType.ModelId,
					"icon" => New.FieldType.Icon,
					"color" => New.FieldType.Color,
					_ => New.FieldType.Scalar,
				},
				Condition = col.Condition,
				Targets = col.Targets,
			};
			if (field.Condition != null || field.Targets != null)
				field.Type = New.FieldType.Link;
	...
	private static (New.Condition? condition, List<string>? targets) ConvertLink(Old.Converter oldLink)
	{
		...
		else if (oldLink.Type == "complexlink")
		{
			...
			var condition = new New.Condition();
