...
namespace DirectoryManager.Extraction;
...
public class EXDExtractor
{
    ...
    private void ParseChunk(SqpkAddData chunk)
    {
        ...
        {
            Log.Debug($"New sparse entry for {chunk.TargetFile}");
    ...
    public void Process(List<string> patchFilePaths, string fileStoragePath, string outputDirectory)
    {
        ...
        Log.Information($"Ready for {patchFilePaths.Count} patch files known as {gameVersion}.");
        ...
        Log.Information($"Processed {patchFilePaths.Count} patch files.");
        ...
        Log.Information($"Processed {_fileIndexStreams.Count} index files.");
        ...
        if (_directory.IndexFiles.Count != 14) // 14 = all but ui script
        {
            ...
            Log.Information($"Filled in {indexWeDontHave.Count} missing index files from {_previousDirectory.Version}.");
        }
        ...
        {
            Log.Information($"Failing processing as root.exl was not found.");
            return;
        }
        
        HandleSqPackFile(rootExlFile, fileStoragePath);
        
        Log.Information($"Loaded root.exl. Contains {rootExlFile.TypedResource.ExdMap.Count} entries.");
        ...
        foreach (var (name, _) in rootExlFile.TypedResource.ExdMap)
        {
            ...
            {
                Log.Information($"Skipped {headerFilePath}, it was null.");
                continue;
            }
    ...
    private SqPackFile<T>? GetSqPackFile<T>(string path) where T : FileResource
    {
        ...
        else
        {
            ...
            int i = 0;
            while (sqPackFile == null && i < _previousDirectories.Count)
            {
            ...
            // Log.Information($"Took {path} from {_previousDirectories[i - 1].Version}");
